...
function setLang(lang) {
    ...
    styleElem.innerHTML = `.side-a::after { content: "${uiText[lang].heads}"; } .side-b::after { content: "${uiText[lang].tails}"; }`;
}
...
window.rollD20 = function() {
    ...
    resultText.style.transform = "translate(-50%, -50%) translateZ(60px) scale(0.5)";

    // 2. Вращаем контейнер
    const x = Math.floor(Math.random() * 720) + 360;
    const y = Math.floor(Math.random() * 720) + 360;
    const z = Math.floor(Math.random() * 360);
    spinner.style.transform = `rotateX(${x}deg) rotateY(${y}deg) rotateZ(${z}deg)`;
    ...
    setTimeout(() => {
        ...
        resultText.style.transform = "translate(-50%, -50%) translateZ(60px) scale(1)";
    }, 1000);
}
